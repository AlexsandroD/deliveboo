    ...
    >
      <template #modal-header="{ close }">
        <div class="d-flex justify-content-between p-sm-3 w-100 ">
          ...
          <button
            type="button"
            class="btn-close ms_close-button"
      ...
      <template #default="{ cancel }">
        ...
        >
          ...
          <div class="row row-cols-1 row-cols-sm-2">
            <div class="col">
              <!-- nome -->
              <label for="name" class="ps-2"
                ><i class="fa-solid fa-user color_primary"></i> Nome *</label
              >
              <input
                v-model="name"
                type="text"
                id="name"
                class="form-control ms_form-control mb-3"
            ...
            <div class="col">
              <!-- cognome -->
              <label for="surname" class="ps-2"
                ><i class="fa-solid fa-user color_primary"></i> Cognome *</label
              >
              <input
                v-model="surname"
                type="text"
          ...
          <!-- email -->
          <label for="email" class="ps-2"
            ><i class="fa-solid fa-envelope color_primary"></i> Email *</label
          >
          <input
            v-model="email"
            type="email"
          ...
          <div class="row">
            <div class="col-12 col-lg-8">
              <!-- indirizzo -->
              <label for="address" class="ps-2"
                ><i class="fa-solid fa-location-dot color_primary"></i>
                Indirizzo *</label
              >
              <input
                v-model="address"
            ...
            <div class="col-12 col-lg-4">
              <!-- telefono -->
              <label for="phone" class="ps-2"
                ><i class="fa-solid fa-phone color_primary"></i> Telefono
                *</label
              >
              <input
                v-model="phone"
          ...
          <!-- commento -->
          <label for="comment" class="form-label ps-2"
            ><i class="fa-solid fa-comment-dots color_primary"></i>
            Commento</label
          >
          <textarea
            v-model="comment"
          ...
          <!-- prezzo totale -->
          <label for="total_price" class="ps-2 my-2"
            >Prezzo totale:
            <strong>&euro;{{ cartLogic.totalPrice }}</strong></label
          >
          ...
          <div class="text-center text-md-end mb-2 braintree_logo">
            ...
            <img
              src="../../assets/images/braintree/braintree-logo-black.png"
              alt="braintree logo"
            />
          </div>

          <!-- numero carta -->
          <label class="ps-2 mt-3 mb-1"
            ><i class="fa-solid fa-credit-card color_primary"></i> Numero carta
            di credito *</label
          >
          <div
            id="creditCardNumber"
            class="form-control ms_form-control"
            required
          ></div>
          <div class="mt-2 ms-2 card_error" v-if="errorNumber">
          ...
          <div class="row row-cols-1 row-cols-sm-2">
            <div class="col">
              <!-- data scadenza -->
              <label class="ps-2 mt-3 mb-1"
                ><i class="fa-solid fa-calendar-day color_primary"></i> Data
                scadenza *</label
              >
              <div id="expireDate" class="form-control ms_form-control"></div>
              <div class="mt-2 ms-2 card_error" v-if="errorExpirationDate">
            ...
            <div class="col">
              <!-- cvv -->
              <label class="ps-2 mt-3 mb-1"
                ><i class="fa-solid fa-lock color_primary"></i> CVV *</label
              >
              <div id="cvv" class="form-control ms_form-control"></div>
              <div class="mt-2 ms-2 card_error" v-if="errorCvv">
                CVV non valido
              </div>
            </div>
          </div>

          <div class="mt-2 ms-2 card_error" v-if="errorEmpty">
            Campi carta vuoti!
          </div>
          ...
          <div class="text-end mt-4">
            <b-button variant="btn btn_close" @click="cancel()"
              >Chiudi</b-button
            >
            <button type="submit" class="btn btn_submit ms-2">
              Conferma e paga
            </button>
          </div>
        </form>
        ...
        <div
          v-show="
            paymentSuccess == true || paymentError == true || showLoader == true
          "
        >
          <p class="fs-4 p-sm-3" v-if="paymentSuccess">
            Pagamento avvenuto con successo. Riceverai una mail con il riepilogo
            dell'ordine!
          </p>
          <p class="fs-4 p-sm-3 payment_error" v-if="paymentError">
            Pagamento fallito, riprovare più tardi.
          </p>
          <div v-if="showLoader">
            <!-- <Loader/> -->
...
export default {
  ...
  methods: {
    onOpen() {
      ...
      axios.get("http://127.0.0.1:8000/api/orders-token").then((response) => {
      ...
      this.hostedFieldInstance
        ...
        .catch((err) => {
          ...
          if (err.code == "HOSTED_FIELDS_FIELDS_EMPTY") {
            err.message =
              "The fields are empty. Please enter your payment information. ";
