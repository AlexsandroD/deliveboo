<template>
  <div class="dropdown_container">
    ...
    <div class="info_container">
      ...
      <!-- lista piatti -->
      <ul class="dish_list" v-if="cartLogic.restaurantId == cartLogic.newRestaurantId">
        <li class="dish" v-for="dish in cartLogic.cart" :key="dish.dishId">
            <div class="d-flex justify-content-between">
              <h6><span class="dish_quantity">{{ dish.quantity }}</span> {{ dish.name }}</h6>
              <span>
                &euro;{{ returnDishTotal(dish.dishPrice, dish.quantity) }}
              </span>
            </div>
            <div class="buttons d-flex">
              <button class="btn_add_minum d-flex justify-content-center align-items-center" @click="cartLogic.removeCartItem(dish.dishId)">
                <i class="fa-solid fa-minus"></i>
              </button>
              <button
                class="btn_add_minum d-flex justify-content-center align-items-center ms-2"
                @click="
      ...
      <!-- totale piatti -->
      <div class="order_total text-end" v-if="cartLogic.restaurantId == cartLogic.newRestaurantId">
      ...
      <div class="d-flex justify-content-end mt-4">
        <button
          ...
          @click="cartLogic.emptyCart()"> Svuota carrello
        </button>
      ...
      <!-- pagamento -->
        <button class="btn ms-2 btn_filled"
